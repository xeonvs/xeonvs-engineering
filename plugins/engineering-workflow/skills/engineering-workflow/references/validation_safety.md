...
Before editing, perform one bounded reconnaissance pass over the independent facts likely to affect the change: applicable instructions and plans, current owners, callers/consumers, existing helpers, tests, and relevant environment dependencies. Reuse still-current facts already available in continuous task context. Overlapping routes do not require rereading one unchanged owner or rerunning one check against the same state at the same boundary. Batch independent reads when their outputs remain attributable. Search first and inspect large sources through focused ranges, but fully ingest every source needed for an exact transformation, safe copy, complete parse, or ownership decision; never edit from truncated content.
...
Define green from the task's named commands and required output conditions. On failure, inspect the relevant evidence, update the hypothesis, make the smallest supported correction, and rerun the affected check. A missing dependency, environmental error, or known unrelated failure is not green; repair it only when authorized and in scope, otherwise report it separately. A failure, truncation, or unread artifact expands inspection only when required evidence remains missing; never hide partial execution behind a summary. When the required checks pass and no task-relevant blocker remains, stop instead of adding optional reads, refactors, delegations, or redundant successful runs. The same check before and after an operation that could change its protected state is not redundant merely because its command text is identical.
...
`Latest Validation Results` records the latest applicable evidence for required checks and scopes. Distinguish an actual command run, a review of whether prior evidence still applies, and a semantic review; do not report one as another. Preserve the real run date, scope, outcome, and necessary provenance.

A material change to a check's implementation, generated input, instruction, acceptance criterion, environment dependency, or other protected input invalidates that evidence for the affected scope. Markdown can be a material input. A plan date, status update, evidence note, or other bookkeeping edit does not by itself invalidate every implementation check, and evidence dated before `Last Updated` is not inherently stale. The root determines applicability from the changed inputs at the existing review and closure boundaries; the lifecycle parser cannot prove that unrecorded code or external state stayed unchanged.

After plan bookkeeping or closure edits, run the checks required for the changed artifacts and preserve still-applicable evidence for unaffected scopes. Do not create a loop in which recording a passed check invalidates that check and forces the same full gate again. A repository's explicit final/full policy, a material final-content change, privacy or secret scanning, terminal-result integrity, and an independent review or immediately-pre-publication boundary still require their specified current evidence.
