...
When a compatible release changes semantics inside the same instruction contract version, first decide whether target-local bytes must change for correctness. If current installed-skill owners already carry the behavior and the prior local template remains compatible, do not add another versioned template archive or fingerprint merely to rewrite it. Register an exact prior pristine fingerprint only when the local owner itself must migrate. Customized owners remain protected and require bounded semantic review when adoption of changed local meaning is in scope: preserve equivalent local rules and ownership, or amend only the missing meaning. A version stamp and structural pass prove structure, not semantic adoption; do not replace that review with phrase matching, a new semantic validator, or a forced rewrite of customized history.
